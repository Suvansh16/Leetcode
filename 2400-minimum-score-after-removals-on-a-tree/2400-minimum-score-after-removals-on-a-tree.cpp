...
void dfs(int node,int parent,vector<int>&subtree,vector<int>&intime,vector<int>&outtime,int &timer,vector<int>&nums,unordered_map<int,vector<int>>&map)
{
    ...
    for(int &nei:map[node])
    {
        if(nei!=parent)
        {
            dfs(nei,node,subtree,intime,outtime,timer,nums,map);
